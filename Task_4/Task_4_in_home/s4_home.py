...
def create_table(name_table):
        ...
        with con.connect() as connection:
                sql.execute(f"""drop table if exists spark_task_4.`{name_table}`""", connection)
                sql.execute(f"""CREATE TABLE if not exists spark_task_4.`{name_table}` (
        ...
        df1 = spark.read.format("com.crealytics.spark.excel")\
                ...
                .option("header", "true")\
                .format("excel")\
                .load("D:/ETL_data_processing/Task_4/Task_4_in_home/Crerdit_calculator.xlsx").limit(1000)\
